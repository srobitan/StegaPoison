...
while len(checked_ckpt) < total_num:
    current_ckpt = [x for x in os.listdir(args.MODEL_DIR) if x.endswith('.pt')]
    current_ckpt = [x for x in current_ckpt if int(x.split('.')[0].split('-')[-1]) % args.SAVE_ROUND == 0]
            ...
            if ckpt not in checked_ckpt:
                ...
                with torch.no_grad():
                    for uid, mask, label in tqdm(val_dataloader):
                        ...
                        # Highly optimized 2D matmul instead of slow 3D BMM expansion
                        bz_score = torch.matmul(user_embedding, total_item_embedding.squeeze(0).t())
